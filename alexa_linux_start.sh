...
--tab -e "bash -ic \"sleep 30s; sudo service mongod stop; sudo docker rm -f /mongo_database;  sleep 5s; sudo docker rm -f mongo; sudo docker run --name mongo_database -d --network myNetwork -p 27017:27017 mongo --noauth; sudo docker start  mongo_database; exec bash\"" \
